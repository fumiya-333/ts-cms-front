<template>
  <FormTemplate>
    ...
    <TextPlane text-class="u-d-block u-px2 u-py1 u-mt2 u-text-sz4"  :value="$v.form.name.$model" placeholder="浦島 太郎" @input="$v.form.name.$model = $event"></TextPlane>
    <div v-if="$v.form.name.$error && !$v.form.name.required" class="u-mt2 u-text-danger">{{ $t('form.name') }}{{ $t('error.required') }}</div>
    ...
    <div class="u-mt10">
      <CreatePreBtn create-pre-btn-class="u-d-block u-w5 u-h1 u-m-x-center u-align-center u-text-sz4" @click="createPre"></CreatePreBtn>
    </div>
  </FormTemplate>
</template>
